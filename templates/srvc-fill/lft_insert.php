...
if (isset($_POST['create'])) {
    ...
    $return_url1 = "lft.php?serial=" . $serial . "&patient_sample=" . $sample . "&report_date=" . $date . "&dr_name=" . $reffered . "&patient_name=" . $name;
    $return_url2 = "&patient_age=" . $age . "&patient_gender=" . $gender . "&sbt=" . $tlc . "&sbd=" . $neu . "&sbi=" . $lym;
    $return_url3 = "&sgot=" . $mono . "&sgpt=" . $eos . "&aklp=" . $bas . "&pt=" . $hb . "&salb=" . $plc . "&glo=" . $rbc . "&ggtp=" . $pcv;
...
if ($udf1 === "no_url" || $udf2 === "no_url" || $udf3 === "no_url" || $udf4 === "no_url") {
    ...
    $action = "http://localhost/HealthKind_OLD/create/verify/index.php";
}
...
<body onload="submitPayuForm()">
    <div class="content">
        <center>
            <br>
            <h class="header">LFT Report<label>
        </center>
        <div class="form">
            <form action="<?php echo $action; ?>" method="POST" name="payuForm" id="reportForm">
                ...
                <input type="number" step="any" min="0" placeholder="Enter SERUM BILIRUBIN (TOTAL)"
                    value="<?php echo $tlc; ?>" name="tlc" />
                <br>
                <label>SERUM BILIRUBIN (DIRECT) :</label>
                <input type="number" step="any" min="0" placeholder="Enter SERUM BILIRUBIN (DIRECT)"
                    value="<?php echo $neu; ?>" name="neu" />
                <br>
                <label>SERUM BILIRUBIN (INDIRECT) :</label>
                <input type="number" step="any" min="0" placeholder="Enter SERUM BILIRUBIN (INDIRECT)"
                    value="<?php echo $lym; ?>" name="lym" />
                <br>
                <label>SGOT :</label>
                <input type="number" step="any" min="0" placeholder="Enter SGOT" value="<?php echo $mono; ?>"
                    name="mono" />
                <br>
                <label>SGPT:</label>
                <input type="number" step="any" min="0" placeholder="Enter SGPT" value="<?php echo $eos; ?>"
                    name="eos" />
                <br>
                <label>ALKALINE PHOSPHATASE :</label>
                <input type="number" step="any" min="0" placeholder="Enter Alkaline Phosphatase "
                    value="<?php echo $bas; ?>" name="bas" />
                <br>
                <label>Protien (TOTAL):</label>
                <input type="number" step="any" min="0" placeholder="Enter Protien(TOTAL)" value="<?php echo $hb; ?>"
                    name="hb" />
                <br>
                <label>Serum Albumin:</label>
                <input type="number" step="any" min="0" placeholder="Enter Serum Albumin" value="<?php echo $plc; ?>"
                    name="plc" />
                <br>
                <label>Glubumin:</label>
                <input type="number" step="any" min="0" placeholder="Enter Glubumin" value="<?php echo $rbc; ?>"
                    name="rbc" />
                <br>
                <label>GGTP:</label>
                <input type="number" step="any" min="0" placeholder="Enter GGTP" value="<?php echo $pcv; ?>"
                    name="pcv" />
                <br>
